...
export default function Question() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const qRes = await axios.get(`/api/questions/${questionid}`, {
      ...
      try {
        const aRes = await axios.get(
          `/api/answers/${questionid}?page=${page}&limit=10`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  useEffect(() => {
    ...
    es.onmessage = async (ev) => {
      try {
        ...
        if (
          data.type === "draft_typing" &&
          data.room === `question:${questionid}`
        ) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        await axios.post(
          "/api/users/collab/typing",
          { room: `question:${questionid}`, text: answer },
          { headers: { Authorization: `Bearer ${token}` } }
        );
      } catch (_) {}
    }, 350);
  ...
  const translateAll = async () => {
    if (!lang) return;
    try {
      const qRes = await axios.post("/api/ai/translate", {
      ...
      for (const a of answers) {
        const aRes = await axios.post("/api/ai/translate", {
          text: a.answer || "",
          target: lang,
        });
  ...
  const submit = async (e) => {
    ...
    try {
      await axios.post(
        "/api/answers",
        { questionid, answer, image_url: imageUrl },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setAnswer("");
      setImageUrl("");
  ...
  const sendChat = async (e) => {
    ...
    try {
      await axios.post(
        "/api/users/chat/send",
        { room: `question:${questionid}`, message: chatMsg },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setChatMsg("");
    } catch (_) {}
  ...
  const loadComments = async (answerid) => {
    try {
      const res = await axios.get(`/api/answers/${answerid}/comments`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    <div className={styles.wrap}>
      <div className={styles.block}>
        ...
        <div className={styles.user}>
          {question?.username} • {formatTimeAgo(question?.created_at)} •{" "}
          {formatDate(question?.created_at)}
        </div>
        <div className={styles.desc}>
          {translatedQuestion || question?.description}
        </div>
        {question?.image_url && (
          <img className={styles.image} alt="" src={question.image_url} />
        )}
      ...
      <div className={styles.block}>
        ...
        <div className={styles.liveBox}>
          ...
          <form onSubmit={sendChat} className={styles.liveForm}>
            <input
              placeholder={t("question.sendMessage")}
              value={chatMsg}
          ...
          <ul className={styles.liveList}>
            {chat.map((m, i) => (
              <li key={i}>
                <span className={styles.commentUser}>{m.username}</span>{" "}
                {m.message}
              </li>
            ))}
          </ul>
        ...
        <ul className={styles.answers}>
          {answers.map((a) => (
            <li key={a.answerid} className={styles.answerItem}>
              ...
              <div className="text">
                {translatedAnswers[a.answerid] || a.answer}
              </div>
              {a.image_url && (
                <img className={styles.image} alt="" src={a.image_url} />
              )}
              <div className={styles.user}>
                {a.username} • {formatTimeAgo(a.created_at)} •{" "}
                {formatDate(a.created_at)}
              </div>
              {question?.username === username && !a.accepted && (
                <button
                  className={styles.btn}
                  onClick={() => accept(a.answerid)}
                >
                  Mark as Best
                </button>
              )}
              {a.accepted ? (
                <span className={styles.badge}>Best Answer</span>
              ) : null}
              <div className={styles.votes}>
                <button
                  className={styles.voteBtn}
                  onClick={() => vote(a.answerid, 1)}
                >
                  ▲
                </button>
              ...
              <div className={styles.comments}>
                <button
                  ...
                  onClick={() => loadComments(a.answerid)}
                >
                  {t("question.showComments") || "Comments"}
                </button>
                <ul className={styles.commentList}>
                  {(commentsByAnswer[a.answerid] || []).map((c) => (
                    <li key={c.commentid} className={styles.commentItem}>
                      ...
                      {c.comment} — {formatTimeAgo(c.created_at)} •{" "}
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    const txt = e.target.elements[`c_${a.answerid}`].value;
                    e.target.reset();
                    addComment(a.answerid, txt);
                  }}
                >
                  <input
                    name={`c_${a.answerid}`}
                    placeholder={
                      t("question.commentPlaceholder") || "Add a comment"
                    }
                  />
                </form>
              </div>
      ...
      <div className={styles.block}>
        ...
        <form className={styles.form} onSubmit={submit}>
          ...
          />
          <input
            placeholder={t("ask.imageUrlOptional")}
            value={imageUrl}
