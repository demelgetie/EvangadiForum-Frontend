...
export default function Ask() {
  ...
  const findRelated = async () => {
    try {
      const res = await axios.get(
        `/api/questions/search/suggest?title=${encodeURIComponent(title)}`
      );
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (title || description) {
        try {
          const tagRes = await axios.post("/api/ai/suggest-tags", {
            title,
            description,
          });
        ...
        try {
          const a2 = await axios.post("/api/ai/write-assist", {
            text: description,
          });
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.card}>
        ...
        <form className={styles.form} onSubmit={submit}>
          ...
          />
          {assistTitle.length > 0 && (
            <div className={styles.assist}>{assistTitle.join(" ")}</div>
          )}
          <textarea
          ...
          />
          {assistDesc.length > 0 && (
            <div className={styles.assist}>{assistDesc.join(" ")}</div>
          )}
          <input
            className={styles.input}
            placeholder={t("ask.tagPlaceholder")}
            value={tag}
          ...
          {related.length > 0 && (
            <ul className={styles.related}>
              {related.map((r) => (
                <li key={r.questionid}>
                  <Link to={`/question/${r.questionid}`}>{r.title}</Link> —{" "}
                  {r.username}{" "}
                  {r.accepted_answerid
                    ? `• ${t("ask.answered")}`
                    : r.answers > 0
                    ? `• ${t("ask.answersCount", { count: r.answers })}`
                    : ""}
                </li>
              ))}
            </ul>
