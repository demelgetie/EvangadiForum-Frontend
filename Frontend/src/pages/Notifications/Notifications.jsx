    ...
    es.onmessage = (ev) => {
      try {
        ...
        if (data.type === "notification") {
          setItems((prev) => [
            { id: Date.now(), type: "live", message: data.message, read: 0 },
            ...prev,
          ]);
        }
          ...
          setItems((prev) => [
            {
              ...
              type: "feed",
              message: t("notifications.newQuestion", { title: data.title }),
              read: 0,
            },
            ...prev,
          ]);
          ...
          setItems((prev) => [
            {
              id: Date.now() + Math.random(),
              type: "feed",
              message: t("notifications.newAnswerBy", {
          ...
          setItems((prev) => [
            {
              id: Date.now() + Math.random(),
              type: "feed",
              message: t("notifications.bestAnswerSelected"),
              read: 0,
            },
            ...prev,
          ]);
      ...
      <ul className={styles.list}>
        {items.map((n) => (
          <li key={n.id} className={styles.item}>
            ...
            {!n.read && (
              <button className={styles.btn} onClick={() => read(n.id)}>
