...
export default function Profile() {
  ...
  useEffect(() => {
    const load = async () => {
      const res = await axios.get(`/api/users/profile/${routeUsername}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const save = async e => {
    e.preventDefault()
    await axios.put('/api/users/profile', { bio, skills, avatarUrl }, { headers: { Authorization: `Bearer ${token}` } })
  }
  ...
  const changePass = async e => {
    ...
    try {
      const res = await axios.post('/api/users/changePassword', { currentPassword, newPassword }, { headers: { Authorization: `Bearer ${token}` } })
      ...
      <div className={styles.grid}>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.avatarWrap}>
              {(avatarUrl || profile?.avatar_url) ? (
                <img alt="avatar" className={styles.avatar} src={avatarUrl || profile?.avatar_url} />
              ) : (
                <div className={styles.avatarPlaceholder}>{(profile?.username || 'U').slice(0,1).toUpperCase()}</div>
              )}
            </div>
            {isOwn && (
              <>
        ...
        <div>
          ...
          <div className={styles.section}>
            ...
            <div className={styles.text}>Questions: {profile?.stats?.questions || 0}</div>
            <div className={styles.text}>Answers: {profile?.stats?.answers || 0}</div>
          </div>
          {isOwn && (
            <div className={styles.section}>
